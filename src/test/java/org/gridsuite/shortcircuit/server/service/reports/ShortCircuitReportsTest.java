...
@Slf4j
@Tag("UnitTest")
class ShortCircuitReportsTest implements WithAssertions, WithCustomAssertions {
    private final ShortCircuitReports shortCircuitReports = new ShortCircuitReports();
    ...
    void testEmptyReporter() {
        assertThat(shortCircuitReports.mapReporter(Reporter.NO_OP)).isInstanceOf(Reporter.NoOpImpl.class).isSameAs(Reporter.NO_OP);
    }

    @Test
    void testIgnoreOthersReportModels() {
        final Reporter reporter = new ReporterModel("test", "Test node");
        assertThat(shortCircuitReports.mapReporter(reporter)).isSameAs(reporter);
    }
    ...
    void testModifyRootNode() {
        ...
        reporter.createSubReporter("generatorConversion", "Conversion of generators")
                .report("disconnectedTerminalGenerator", "Regulating terminal of connected generator ${generator} is disconnected. Regulation is disabled.", "generator", "TestGenerator");
        assertThat(shortCircuitReports.mapReporter(targetReporter))
    ...
    void testModificationLogs() throws JsonProcessingException {
        ...
        final Reporter result = shortCircuitReports.mapReporter(rootReporter);
