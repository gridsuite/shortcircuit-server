 ...
 */
@Tag("IntegrationTest")
class ReporterLogsIT {
    @Test
    void testReportMapperServiceIsCalled() throws Exception {
        final ReportMapperService reportMapperServiceMocked = Mockito.mock(ReportMapperService.class);
        ...
        try (final MockedStatic<ShortCircuitAnalysis> shortCircuitAnalysisMockedStatic = TestUtils.injectShortCircuitAnalysisProvider(providerMock);
            ...
            Mockito.when(reportMapperServiceMocked.modifyReporter(any(ReporterModel.class))).thenReturn(reporter);
            final ShortCircuitWorkerService workerService = new ShortCircuitWorkerService(networkStoreServiceMocked, reportServiceMocked, notificationServiceMocked, resultRepositoryMocked, objectMapperMocked, reportMapperServiceMocked);
            workerService.consumeRun().accept(message);
            shortCircuitAnalysisMockedStatic.verify(ShortCircuitAnalysis::find, atLeastOnce());
            Mockito.verify(reportMapperServiceMocked, times(1)).modifyReporter(any(ReporterModel.class));
